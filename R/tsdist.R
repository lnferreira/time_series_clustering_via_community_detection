...
# ==============================================================================
# Dist Map-Reduce Naive
# ==============================================================================
dist.map <- function(tsList, part, numParts, distFunc=tsdiss.euclidean, 
                     saveDist=TRUE, filePathName="./dist") {
    tsListLength = length(tsList)
    dist = matrix(0, tsListLength, tsListLength)
    combs = combn(tsListLength, 2)
    l = ncol(combs) / numParts
    l = (1:l)+((part-1)*l) 
    combs = combs[,l]
    distFuncCompiled <- cmpfun(distFunc)
    for(l in 1:ncol(combs)) {
        cat("\r", "calculating distance part: ", part, "/", numParts, ", tsDists:", l, "of", ncol(combs))
        i = combs[1, l]
        j = combs[2, l]
        dist[i,j] = dist[j,i] = distFuncCompiled(tsList[[i]], tsList[[j]])
    }
    if(saveDist) {
        measureName = tolower(strsplit(deparse(substitute(distFunc)), "\\.")[[1]][2])
        filePathName = paste0(filePathName, "_", measureName, "_", part, "_of_",numParts,".RDS")
        cat("\nsaving file:", filePathName, "\n")
        saveRDS(dist, filePathName)
    }
    dist
}

dist.reduce <- function(dataset, fileName, filePath=".", filePattern="dist", saveDist=TRUE, distFunc=NULL, askContinue=TRUE) {
    files = sort(list.files(filePath, filePattern,full.names=T))
    cat("reducing the following files:\n")
    for (f in files) {
        cat(f, "\n")
    }
    opt = NULL
    ...
    dist = 0
    if (tolower(opt) == "y") {
        dist = matrix(0, length(dataset), length(dataset))
        for (i in 1:length(files)) {
            dist = dist + readRDS(files[i])
        }
        if (length(which(dist==0))) {
            distFuncCompiled <- cmpfun(distFunc)
            missing = which(dist==0, arr.ind=T)
            numMissing = nrow(missing)
            for (l in 1:numMissing){
                cat("\rcalculating missed distances:", l, "/", numMissing)
                i = missing[l, 1]
                j = missing[l, 2]
                if(i != j) {
                    dist[i,j] = dist[j,i] = distFuncCompiled(dataset[[i]], dataset[[j]])
                }
            }
        }
        cat("\nsaving dist file:", fileName, "\n")
        saveRDS(dist, fileName)
    }
    dist
}

dist.maps <- function(tsList, part, numParts, functionList, filePathName="./data_dists/ncep/parts/dist_") {
    for (i in 1:length(functionList)) {
        func = functionList[[i]]$f
        funcName = functionList[[i]]$name
        cat("calculating func", funcName, " ", i, "of", length(functionList), "\n")
        dist.map(tsList, part, numParts, func, T, paste0(filePathName, funcName))
    }
}
